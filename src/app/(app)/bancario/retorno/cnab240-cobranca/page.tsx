...
export default function RetornoCnab240CobPage() {
  ...
  const handleProcess = () => {
    if (!file) return;
    setProcessed(true);
    toast({ title: "Retorno CNAB 240 Cobrança processado!", description: `${mockRetorno.filter((r) => r.status === "RECEIVED").length} recebimento(s) confirmado(s).` });
  };
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold tracking-tight">Retorno CNAB 240 — Cobrança</h1>
        <p className="text-sm text-muted-foreground">Integrações Bancárias › Retorno › CNAB 240 Cobrança</p>
      </div>
      <div className="flex items-center gap-2 p-3 rounded-lg bg-indigo-50 border border-indigo-200 text-sm text-indigo-800">
        ...
        <span>Processa arquivo de retorno <strong>CNAB 240</strong> de cobrança (segmentos T e U), confirmando recebimentos e atualizando saldos.</span>
      </div>
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select defaultValue="BOL"><SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
              ...
              <table className="w-full text-sm">
                <thead className="border-b bg-muted/30">
                  <tr>
                    ...
                    <th className="py-3 px-4 text-center font-medium text-muted-foreground">Segmento</th>
                ...
                <tbody className="divide-y">
                  {mockRetorno.map((r) => (
                    <tr key={r.id} className="hover:bg-muted/20">
                      ...
                      <td className="py-3 px-4 text-center font-mono text-xs">{r.segmento}</td>
